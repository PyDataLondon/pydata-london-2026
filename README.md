...
Tell your LLM to `fetch https://london.pydata.org/pydata-london-2026/recommender.md and follow the instructions` and it becomes a personalised conference recommender: top picks, a day-by-day itinerary, and a "stretch" pick to break you out of your bubble.

- 🌐 Landing page: <https://london.pydata.org/pydata-london-2026/>
- 📄 The artifact LLMs fetch: <https://london.pydata.org/pydata-london-2026/recommender.md>
- 🎟 Tickets: <https://ti.to/pydata/pydatalondon26>

> **Heads up:** the eventual short URL is `https://pydata.london/` (domain to be registered). Until then the working URL is the `london.pydata.org` one above.
...
## Custom domain (when `pydata.london` is registered)

Today the site is served from the org-level CNAME at `london.pydata.org` (so the project URL is `https://london.pydata.org/pydata-london-2026/`).

To switch to a freshly-bought `pydata.london`:
...
4. Update the two `RECOMMENDER_URL` / `LANDING_URL` constants in `scripts/build_recommender.py` and the `PROMPT` constant + visible prompt span in `index.html`.
5. Re-run the build script and commit.
